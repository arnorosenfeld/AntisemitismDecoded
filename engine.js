...
// Distribute trust bonus proportionally — more aligned segments get more
function applyTrustBonusProportionally(bonus) {
  var segments = GAME_DATA.config.segments || [];
  if (!segments.length || !G.segmentApproval) return;
  var orgPos = G.politicalPosition || 50;

  // Calculate alignment scores: closer to org position = higher share
  var alignScores = [];
  var totalAlign = 0;
  segments.forEach(function(seg) {
    var dist = Math.abs(orgPos - seg.politicalCenter);
    var comfort = seg.comfortRange || 15;
    // Score: 1.0 at center, tapering to 0.2 at far distance
    var score;
    if (dist <= comfort / 2) {
      score = 1.0;
      ...
      score = Math.max(0.1, 0.3 - (dist - comfort) / 100);
    }
    alignScores.push(score);
    totalAlign += score;
  });

  // Distribute bonus proportionally
  segments.forEach(function(seg, i) {
    var share = (alignScores[i] / totalAlign) * bonus * segments.length;
    G.segmentApproval[seg.id] = Math.max(5, Math.min(95, Math.round((G.segmentApproval[seg.id] || 40) + share)));
  });
}
...
function startGame(){
  ...
  // Apply trust bonuses from setup proportionally across segments
  var trustBonus = (G._baselines.trust || 0) + (G.priorities.trust || 0);
  if (trustBonus > 0 && G.segmentApproval) {
    applyTrustBonusProportionally(trustBonus);
  }

  // Enforce floor: if composite trust is below 30, boost aligned segments
  G.stats.trust = computeCompositeTrust();
  var TRUST_FLOOR = 30;
  var attempts = 0;
  while (G.stats.trust < TRUST_FLOOR && attempts < 20) {
    applyTrustBonusProportionally(2); // small increments to aligned segments
    G.stats.trust = computeCompositeTrust();
    attempts++;
  }
...
function startPromoGame(){
  ...
  // Apply trust bonuses proportionally
  var trustBonus = (G._baselines.trust || 0) + (G.priorities.trust || 0);
  if (trustBonus > 0 && G.segmentApproval) {
    applyTrustBonusProportionally(trustBonus);
  }
  ...
  G.stats.trust = computeCompositeTrust();
  var attempts = 0;
  while (G.stats.trust < 30 && attempts < 20) {
    applyTrustBonusProportionally(2);
    G.stats.trust = computeCompositeTrust();
    attempts++;
  }
...
function beginRound(){
  ...
  // Between-round flow: ticker update → main (coalition offers now on end-of-round screen only)
  if(G.round > 1) {
    feedTickerHeadlines();
  }
  afterCoalitionCheck();
...
function renderMain(){
  ...
  var monScreen = '<div class="desk-mon-desktop">'
    ...
    + '<div class="desk-mon-win-bar desk-mon-win-bar-green"><span class="desk-mon-dot desk-mon-dot-r"></span><span class="desk-mon-dot desk-mon-dot-y"></span><span class="desk-mon-dot desk-mon-dot-g"></span><span class="desk-mon-win-title">Gelt — Q' + G.round + '</span></div>'
    ...
    + '</div>'
    + '<div class="desk-mon-taskbar"><div class="desk-mon-taskbar-item active">Gelt</div><div class="desk-mon-taskbar-item">News</div><div class="desk-mon-taskbar-item">Mail</div></div>';
...
function showRoundTransition() {
  ...
  if (pendingCoalition) {
    ...
    Object.entries(breakFx).forEach(function(e) { var lbl = (GAME_DATA.stats.find(function(s){return s.id===e[0]})||{}).label || e[0]; if(e[1] < 0) breakParts.push(e[1] + ' ' + lbl); });
    if (breakBudget < 0) breakParts.push(breakBudget + ' Gelt');
    var breakText = breakParts.length ? breakParts.join(', ') : '';
    ...
    coalitionHtml = '<div style="display:flex;align-items:center;gap:16px;margin:24px 0"><div style="flex:1;height:1px;background:var(--border)"></div><span style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--muted)">Coalition Opportunity</span><div style="flex:1;height:1px;background:var(--border)"></div></div>' +
      '<div id="rt-coalition" style="background:var(--white);border:2px solid var(--gold);padding:20px 24px;margin-bottom:24px">' +
        ...
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">' +
          '<div style="background:#ede8dc;padding:10px 12px"><div style="font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--muted);margin-bottom:4px">Benefits (per round)</div><div style="font-size:12px;color:#2d6a4f">' + benefitParts.join(', ') + '</div></div>' +
          '<div style="background:#ede8dc;padding:10px 12px"><div style="font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--muted);margin-bottom:4px">Constraint</div><div style="font-size:12px;color:#c0392b">' + esc(constraintText) + '</div></div>' +
        '</div>' +
        (breakText ? '<div style="background:#f5e6e6;padding:8px 12px;margin-bottom:16px;font-size:12px;color:#8b1a1a"><strong>If broken:</strong> ' + breakText + '</div>' : '') +
...
function showGazette() {
  ...
  var storiesHtml = headlines.map(function(h){
    return '<div class="gazette-story"><div class="gazette-headline">' + esc(substituteOrgTokens(h.headline)) + '</div><div class="gazette-subline">' + esc(substituteOrgTokens(h.subline || '')) + '</div></div>';
  }).join('');
...
function substituteOrgTokens(s) {
  if (!s) return '';
  var name = getOrgName();
  return String(s).replace(/\{orgName\}/g, name).replace(/\[org\]/g, name);
}
function pickGazetteHeadlines() {
  ...
  (G.notableMoments || []).forEach(function(m) {
    if (m.round === G.round && m.headline) {
      outcomeHeadlines.push({
        headline: substituteOrgTokens(m.headline),
        subline: substituteOrgTokens(m.subline || '')
      });
    }
  });
